...
<!-- BADGES -->
<a href="#"><img src="https://img.shields.io/badge/Phiên_bản-1.4.0-EE0033?style=for-the-badge&logo=semanticweb&logoColor=white" alt="Version"/></a>
&nbsp;
...
&nbsp;
<img src="https://img.shields.io/badge/Tự_động_hóa-3_Module-EE0033?style=flat-square"/>
&nbsp;
<img src="https://img.shields.io/badge/Mạng_nội_bộ-ViettelPost-00A859?style=flat-square"/>
&nbsp;
<img src="https://img.shields.io/badge/Cài_đặt-30_giây-6366f1?style=flat-square"/>
...
| # | Mục | Mô tả |
|:-:|-----|-------|
| 1 | [🌟 Tính năng](#-tính-năng) | 3 module tự động hóa tích hợp trong một giao diện |
| 2 | [🚀 Cài đặt](#-hướng-dẫn-cài-đặt) | 4 bước đơn giản, không cần lập trình |
| 3 | [💡 Sử dụng](#-cẩm-nang-sử-dụng) | Hướng dẫn chi tiết từng module |
| 4 | [📂 Kiến trúc](#-cấu-trúc-mã-nguồn) | Sơ đồ luồng và cấu trúc file |
...
> Một Chrome Extension duy nhất — **3 module tự động hóa** phục vụ toàn bộ quy trình vận hành ViettelPost.
...
**📊 Tiến trình thực**
Progress bar + counter hiển thị trực tiếp trên popup theo thời gian thực.

**🔕 Không Alert gián đoạn**
Toast Notification tự động đóng sau 3 giây thay cho alert() nguyên bản.
...
**🏷️ Lọc mã thông minh**
Nhận diện các đầu mã hợp lệ: `SHOPEE`, `VTP`, `PKE`, `KMS`, `PSL`, `TPO` (có thể mở rộng).
...
Bấm **`Tải tiện ích đã giải nén`** → chọn thư mục vừa giải nén.

<br/>

> [!TIP]
> ✅ **Xong!** Nhấn icon 📌 **ghim (pin)** tiện ích lên thanh công cụ để mở nhanh.

> [!IMPORTANT]
> Extension chỉ hoạt động trên **mạng nội bộ ViettelPost**. Khi test, có thể dùng server giả lập trong thư mục `test_server/`.
...
```
[1] Mở popup  →  [2] Chọn tab "Sửa Giờ Lấy Hàng"  →  [3] Dán mã vận đơn
                                                              (mỗi mã 1 dòng)
                                                                    ↓
[5] Bấm ▶ BẮT ĐẦU CHẠY  ←────────────────────────  [4] Cài Độ Trễ (mặc định 4s)
          ↓
    Tool ẩn đi và tự xử lý tuần tự từng mã trên tab hiện tại 🤖
```
...
[1] Mở trang kiểm kê bưu phẩm ViettelPost
         ↓
[2] Bấm icon tiện ích → Chọn tab "Kiểm Kê Tồn Tuyến"
         ↓
[3] Bấm "Tải danh sách tuyến từ VTP"
         ↓           ↓
    Load tuyến    Hiển thị checklist
         ↓
...
[1] Mở trang kiểm kê bưu phẩm (tab "Bưu phẩm chưa kiểm kê")
         ↓
[2] Bấm icon tiện ích → Tab "Kiểm Kê Tồn Tuyến"
         ↓
...
│
├── 📄 manifest.json          ← [CORE]    Cấu hình Extension MV3, khai báo quyền
├── 📄 popup.html             ← [UI]      Giao diện popup 2 tab (Be Vietnam Pro font)
├── 📄 popup.css              ← [UI]      ViettelPost brand system, 8px grid
├── 📄 popup.js               ← [LOGIC]   Điều phối tab, inject script, vòng lặp tuyến
│
├── 📄 notification.js        ← [SHARED]  Toast Notification custom (thay thế alert)
│
├── 📄 chinhgio_content.js    ← [MODULE1] Engine sửa giờ lấy hàng theo lô
│
├── 📄 kiemke_tuyen_auto.js   ← [MODULE2] Thực hiện 5 bước chọn tuyến → vào trang scan
├── 📄 gapton_settings.js     ← [MODULE3] Cấu hình prefix mã hợp lệ (cache)
├── 📄 gapton_smart_delay.js  ← [MODULE3] Chờ DOM thay đổi sau khi quét mã
└── 📄 gapton_core_scan.js    ← [MODULE3] Engine quét mã + HUD Overlay + phân trang
```
...
```mermaid
graph TD
    User["🖱️ Người dùng\nbấm popup"] --> Tab{Chọn Tab}

    Tab -->|"Sửa Giờ"| CG["popup.js\nInject chinhgio_content"]
    Tab -->|"Kiểm Kê Tuyến"| KK["popup.js\nVòng lặp tuyến"]

    CG --> CGE["chinhgio_content.js\n🔄 Xử lý hàng loạt"]
    CGE --> CGD["gapton_smart_delay.js\n⏱️ Chờ DOM ổn định"]
    CGD --> CGR["✅ Cập nhật giờ từng đơn"]
    ...
    KKC --> KKD{Còn tuyến?}
    KKD -->|"Có"| KKA
    KKD -->|"Xong"| Done["✅ Hoàn tất tất cả tuyến"]

    style User fill:#EE0033,color:#fff,stroke:none
    style CGR fill:#00A859,color:#fff,stroke:none
...
**Giải pháp:** Nhấn `F5` tải lại trang, sau đó mở lại popup tiện ích.
